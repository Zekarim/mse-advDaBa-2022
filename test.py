...
class Example(object):
            ...
            if 'authors' in item:
                for author in item['authors']:
                    ...
                    if 'name' in author and '_id' in author:
                        with driver.session() as session:
                            session.run(
                                "MERGE (b:Author {_id: $id, name: $name}) " +
                                "WITH b " +
    ...
    def process_relation(self, data):
    ...
    def main(cls, args):
        ...
        thread1 = threading.Thread(
            target=ex.process_relation, args=(data[:quart],))
        thread2 = threading.Thread(
            target=ex.process_relation, args=(data[quart:quart*2],))
        thread3 = threading.Thread(
            target=ex.process_relation, args=(data[quart*2:quart*3],))
        thread4 = threading.Thread(
            target=ex.process_relation, args=(data[quart*3:quart*4],))
        thread5 = threading.Thread(
            target=ex.process_relation, args=(data[quart*4:quart*5],))
        thread6 = threading.Thread(
            target=ex.process_relation, args=(data[quart*5:quart*6],))
        thread7 = threading.Thread(
            target=ex.process_relation, args=(data[quart*6:quart*7],))
        thread8 = threading.Thread(
            target=ex.process_relation, args=(data[quart*7:],))
